    ...
    return Scaffold(
      body: WillPopScope(
        ...
        child: SafeArea(
          child: Column(
            children: <Widget>[
              if (isLoading) LinearProgressIndicator(), // Aquí está el nuevo indicador de progreso
              Expanded(
                child: Stack(
                  children: [
                    if (isConnected)
                      InAppWebView(
                        ...
                        onLoadStart: (controller, url) {
                          setState(() {
                            isLoading = true; // Comienza la carga, activa el indicador de progreso
                          });
                        },
                        onLoadStop: (controller, url) {
                          setState(() {
                            isLoading = false; // Termina la carga, desactiva el indicador de progreso
                          });
                          // Limpiar el contador de retroceso cuando se carga una nueva página
                          backPressCount = 0;
                        },
